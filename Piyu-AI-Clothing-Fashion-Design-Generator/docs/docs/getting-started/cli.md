---
sidebar_position: 4
title: Unified CLI (piyu)
description: Run every Piyu adapter -- virtual try-on, image/video generation, editing, understanding, and background removal -- from a single piyu command.
keywords:
  - piyu CLI
...
Once Piyu is installed (`pip install -U piyu` or `pip install -e .`;
**v0.0.3+** recommended), every adapter in the repo is available through a
single `piyu` command with three levels of control: **service** &rarr;
...
`pip install piyu[local]`; everything else calls a cloud API and needs
...
```bash
piyu --help                              # list services
piyu understand --help                   # list models for a service
piyu understand --model kimi-k2.6 --help # list that model's parameters
```
...
# Text-to-image
piyu generate --model nano-banana-pro \
...
piyu understand --model kimi-k2.6 \
...
piyu video-generate --model seedance --prompt "10s lookbook walk" --duration 10
piyu video-generate --model kling-v3 --prompt "atelier pan" --mode pro --sound on
piyu video-generate --model luma-ray-3.2 --prompt "dolly through mist" --resolution 720p
piyu video-generate --model grok-imagine-video --prompt "cinematic push-in" --duration 6
piyu video-generate --model p-video --prompt "runway walk, soft light" --duration 5
piyu video-generate --model p-video-replace --video clip.mp4 --images identity.jpg
piyu video-generate --model p-video-avatar --image portrait.jpg --voice-script "Hello from the showroom."
piyu video-generate --model p-video-animate --video driver.mp4 --image subject.jpg

# Seedream / Ideogram / Grok Imagine Image / Pruna P-Image
piyu generate --model seedream --prompt "editorial sneaker still" --size 2K
piyu generate --model ideogram --prompt 'Poster "SUMMER 2026"' --rendering-speed QUALITY
piyu generate --model grok-imagine-image --prompt "street-art collage" --aspect-ratio 16:9
piyu generate --model p-image --prompt "luxury knitwear flatlay" --aspect-ratio 1:1
piyu edit --model p-image-edit --images photo.jpg --prompt "clean studio background"
piyu edit --model p-image-upscale --image photo.jpg --target 8

# Background removal
piyu bg-remove --model ben2 --image product.jpg --refine
```
