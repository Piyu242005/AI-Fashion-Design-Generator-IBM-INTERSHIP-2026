...
HF_TOKEN=os.getenv("HF_TOKEN","").strip(); MAX_BYTES=10*1024*1024; CONNECT_TIMEOUT=20; QUEUE_TIMEOUT=95
...
def _parse_multipart(content_type,raw):
 ...
 fields={}
 def on_field(field):
  name=field.field_name.decode() if isinstance(field.field_name,bytes) else str(field.field_name); value=field.value
 ...
 # python-multipart 0.0.9+ expects (boundary, callbacks, max_size=...).
 parser=MultipartParser(boundary.encode(),{"on_field":on_field,"on_file":on_file},max_size=1<<24)
...
def _submit(pf,gf,desc):
 data=[{"background":pf,"layers":[],"composite":None},gf,desc[:200] or "fashion garment",True,True,30,42]
 ...
 if not event:raise RuntimeError(f"No event_id returned by IDM-VTON: {payload!r}")
 return event
def _wait(event):
 req=Request(f"{SPACE_URL}/gradio_api/call/tryon/{event}",headers={**_headers(),"Accept":"text/event-stream"},method="GET")
 ...
 with _http(req,QUEUE_TIMEOUT) as r:
  ...
  while time.monotonic()<deadline:
   ...
   while "\n\n" in buffer:
    ...
    if ev=="error":raise RuntimeError(str(data))
